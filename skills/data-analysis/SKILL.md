---
name: data-analysis
description: Analyze data and produce professional reports with Python. Use this skill whenever the user asks to analyze CSV, Excel, SQL results, business metrics, KPIs, or any tabular data — including requests like "look at this data", "what trends do you see", "make a chart from this", "summarize these numbers", or "write a report from this spreadsheet". Also use for research synthesis when the user provides multiple sources (articles, notes, search results) and wants structured findings. Also use when the user mentions database tables (ODPS, BigQuery, MySQL, etc.) and wants analysis — this skill handles the analysis planning, data requirements, and report generation, coordinating with query tools for actual data retrieval. Covers the full pipeline from data requirements definition through sufficiency checks, Python-based analysis with charts, to a polished Markdown report with limitations and source appendix.
hooks:
  PreToolUse:
    - matcher: "Bash"
      hooks:
