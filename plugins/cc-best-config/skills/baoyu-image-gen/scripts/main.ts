...
const DEFAULT_PROVIDER_RATE_LIMITS: Record<Provider, ProviderRateLimit> = {
  ...
  modelscope: { concurrency: 3, startIntervalMs: 1100 },
...
Options:
  ...
  --provider google|openai|openrouter|dashscope|modelscope|replicate|jimeng|seedream  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  MODELSCOPE_IMAGE_MODEL    Default ModelScope model (Qwen/Qwen-Image-2512)
  ...
  MODELSCOPE_BASE_URL       Custom ModelScope endpoint
...
export function getConfiguredProviderRateLimits(
  ...
  const configured: Record<Provider, ProviderRateLimit> = {
    ...
    modelscope: { ...DEFAULT_PROVIDER_RATE_LIMITS.modelscope },
    jimeng: { ...DEFAULT_PROVIDER_RATE_LIMITS.jimeng },
    seedream: { ...DEFAULT_PROVIDER_RATE_LIMITS.seedream },
  };

  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "modelscope", "jimeng", "seedream"] as Provider[]) {
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, MODELSCOPE_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, or ARK_API_KEY.\n" +
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "modelscope") return (await import("./providers/modelscope")) as ProviderModule;
...
function getModelForProvider(
  ...
  if (extendConfig.default_model) {
    ...
    if (provider === "modelscope" && extendConfig.default_model.modelscope) return extendConfig.default_model.modelscope;
